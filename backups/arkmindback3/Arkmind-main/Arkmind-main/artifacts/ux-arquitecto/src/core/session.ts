...
export class SessionManager {
  ...
  forkSession(
    ...
    const messagesSlice = original.messages.slice(0, fromIndex + 1);
    ...
    const forked: AIContextSession = {
      ...original,
      id: forkedId,
      title: options?.title ?? (original as any).title,
      state: "forked",
      messages: deepClone(messagesSlice),
      proposals: deepClone(original.proposals || []),
      createdAt: Date.now(),
      lastActive: Date.now(),
      metadata: {
        ...original.metadata,
