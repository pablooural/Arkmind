...
export const coreEngine = {
  ...
  async hydrateAll(): Promise<void> {
    const tasks = [
      { name: "sessions", run: () => sessionManager.hydrate() },
      { name: "cognitive", run: () => cognitiveManager.hydrate() },
      { name: "visual", run: () => visualManager.hydrate() },
      { name: "memory", run: () => memoryManager.hydrate() },
    ];
    await Promise.all(
      tasks.map(async ({ name, run }) => {
        try {
          ...
          console.error(`[coreEngine] hydrate failed for ${name}:`, error);
        }
      }),
    );
  },
